...
# Inputs and paths
passed_path="${1:-}"
# awww >=0.12.0 stores cache under a versioned subdirectory; fall back to root for older builds
_awww_ver=$(awww --version 2>/dev/null | awk '{print $2}')
if [[ -d "$HOME/.cache/awww/$_awww_ver" ]]; then
  cache_dir="$HOME/.cache/awww/$_awww_ver/"
else
